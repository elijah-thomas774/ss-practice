...
impl<const NUM_LINES: usize, const LINE_SIZE: usize> SimpleMenu<NUM_LINES, LINE_SIZE> {
    ...
    pub fn move_cursor(&mut self, cursor: u32) -> u32 {
        self.set_cursor(cursor);
        if is_pressed(DPAD_UP) {
            let len = self.line_buf.len() as u32;
            self.current_line = (self.current_line + len - 1) % len;
        } else if is_pressed(DPAD_DOWN) {
            let len = self.line_buf.len() as u32;
            self.current_line = (self.current_line + 1) % len;
        }
        self.current_line
    }
